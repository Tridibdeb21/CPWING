...
import { Calculator, CheckCircle2, Upload } from 'lucide-react'
...
const AdminContest = () => {
  ...
  const [contest, setContest] = useState({ name: '', date: '', month: '' })
  ...
  useEffect(() => {
    if (!isAdmin) return
    supabase.from('contests').select('id, name, contest_date, month').eq('status', 'published').order('contest_date', { ascending: true }).then(({ data }) => {
  ...
  const createContest = async () => {
    setError('')
    setStatus('')
    if (!contest.name || !contest.date || !contest.month) {
      setError('Enter a contest name, date, and rating month first.')
      return
    }
    setSaving(true)
    const { data, error: contestError } = await supabase.from('contests').insert({ name: contest.name, contest_date: new Date(`${contest.date}T00:00:00`).toISOString(), month: `${contest.month}-01`, status: 'published' }).select('id, name, contest_date, month').single()
    ...
    <div className="container dashboard-page animate-fade-in">
      ...
      <section className="admin-layout">
        <div className="glass-panel admin-form">
          ...
          <button className="btn-secondary" type="button" onClick={createContest} disabled={saving}>Create contest for registration</button>
          <label htmlFor="existing-contest">Contest to rate</label>
          <select id="existing-contest" value={selectedContestId} onChange={(event) => setSelectedContestId(event.target.value)}>
            <option value="">Select a created contest</option>
            {availableContests.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.month.slice(0, 7)})</option>)}
          </select>
