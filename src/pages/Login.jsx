...
import { ArrowRight, CheckCircle2, ExternalLink, KeyRound, LoaderCircle, LogIn, UserPlus } from 'lucide-react'
...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const { error: resetError } = await supabase.functions.invoke('reset-password-with-codeforces', {
          body: { email, codeforces_handle: forgotHandle, submission_id: forgotSubmissionId, new_password: newPassword }
        })
        if (resetError) {
        ...
        if (codeforcesStatus !== 'verified') throw new Error('Verify your Codeforces compilation error before submitting registration.')
    ...
    if (result.error) {
      const signupDisabled = mode === 'signup' && (result.error.code === 'email_provider_disabled' || result.error.message.toLowerCase().includes('email signups are disabled'))
      setError(signupDisabled
        ? 'Email signup is disabled in Supabase. Enable the Email provider and Allow new users to sign up in Authentication settings.'
        : result.error.message)
    } else if (mode === 'login') {
      navigate(location.state?.from?.pathname || '/dashboard', { replace: true })
    } else if (!result.data.session) {
      setError('Email confirmation is still enabled in Supabase. Disable Confirm email in Authentication settings, then try again.')
    } else {
      ...
      if (profileError) {
        setError(`Verification succeeded, but the profile could not be saved: ${profileError.message}`)
      } else {
        navigate(location.state?.from?.pathname || '/dashboard', { replace: true })
      }
    }
  ...
  return (
    <div className="container auth-page animate-fade-in">
      <div className="auth-panel glass-panel">
        ...
        {mode === 'login' && <button className="auth-switch" type="button" onClick={openForgotPassword}>Forgot password?</button>}
