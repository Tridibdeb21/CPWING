...
const Home = () => {
  ...
  return (
    <div className="container animate-fade-in" style={{ paddingTop: 'clamp(3rem, 8vw, 6rem)', paddingBottom: 'clamp(3rem, 8vw, 6rem)' }}>
      ...
      <div className="activity-title-wrap" style={{ marginBottom: '1.6rem' }}>
        <span className="activity-title-kicker">Running Activities</span>
        <h2 className="activity-title">Running Activities</h2>
      </div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: '2rem', marginBottom: '6rem' }}>
        <motion.div variants={itemVariants} className="glass-panel" style={{ display: 'block', textDecoration: 'none', color: 'var(--text-primary)' }}>
          <Link to="/level0" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textDecoration: 'none', color: 'inherit' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
              <Code size={32} color="var(--accent-purple)" />
            </div>
            <h3 style={{ fontSize: '1.5rem', marginTop: '0.5rem' }}>Level 0</h3>
            <p style={{ color: 'var(--text-secondary)' }}>Start from zero with C++ fundamentals, conditions, loops, arrays, and guided contests designed for beginners.</p>
          </Link>
        </motion.div>

        <motion.div variants={itemVariants} className="glass-panel" style={{ display: 'block', textDecoration: 'none', color: 'var(--text-primary)' }}>
          <Link to="/level1" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textDecoration: 'none', color: 'inherit' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
              <Zap size={32} color="#f59e0b" />
            </div>
            <h3 style={{ fontSize: '1.5rem', marginTop: '0.5rem' }}>Level 1</h3>
            <p style={{ color: 'var(--text-secondary)' }}>Move into problem-solving patterns like prefix sum, two pointers, STL, and number theory with practice contests.</p>
          </Link>
        </motion.div>
      </motion.div>
